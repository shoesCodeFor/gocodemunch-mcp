...
- [x] Add tests and execute the full benchmark matrix:
  ...
  - Completed on 2026-05-02: added token-savings unit coverage for telemetry snapshot trend rollups and direct Markdown artifact rendering, plus a real `gocodemunch-eval` integration test that exercises both modes and all competitors through the actual embedder/backend factories.
  - Completed on 2026-05-02: fixed the Qdrant adapter so arbitrary chunk IDs are deterministically rewritten to valid UUID-like point IDs for storage while preserving original IDs in query/delete results, which removed the Qdrant 400s surfaced by the live benchmark matrix run.
  - Completed on 2026-05-02: executed smoke and full matrix token-savings benchmarks end to end with the local eval embed stub and Qdrant, producing JSON artifacts at `Auto Run Docs/Working/evals/token-savings-smoke.json` and `Auto Run Docs/Working/evals/token-savings-matrix.json`, plus Markdown artifacts under `Auto Run Docs/Working/evals/savings-runs/` with `Auto Run Docs/Working/evals/Savings-Index.md`.
  - Verified with `go test ./src/internal/storage/vector/qdrant ./src/cmd/gocodemunch-eval -count=1`, `go test ./src/... ./tests-go -count=1`, `go vet ./src/cmd/gocodemunch-eval ./src/internal/storage/vector/qdrant`, `go run ./src/cmd/gocodemunch-eval --mode token-savings-smoke --suite-path tests-go/evals/fixtures/token-savings-smoke --providers ollama --backends sqlite --output-path "Auto Run Docs/Working/evals/token-savings-smoke.json" --markdown-report-dir "Auto Run Docs/Working/evals/savings-runs" --skip-markdown-report=false`, and `go run ./src/cmd/gocodemunch-eval --mode token-savings-smoke --suite-path tests-go/evals/fixtures/token-savings-smoke --providers ollama,vllm --backends sqlite,qdrant --output-path "Auto Run Docs/Working/evals/token-savings-matrix.json" --markdown-report-dir "Auto Run Docs/Working/evals/savings-runs" --skip-markdown-report=false`.
