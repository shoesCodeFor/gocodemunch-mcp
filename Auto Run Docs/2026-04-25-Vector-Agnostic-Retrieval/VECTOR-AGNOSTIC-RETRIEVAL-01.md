...
- [x] Add `src/internal/domain/indexing/vector_contracts.go` with interfaces for `Upsert`, `Query`, `Delete`, `DeleteNamespace`, and `Health(ctx)` plus embedder contract and retryable error classification.
  - Completed: Added composable vector backend contracts (`VectorUpserter`, `VectorQuerier`, `VectorDeleter`, `VectorNamespaceDeleter`, `VectorHealthChecker`) and aggregate `VectorBackend`, plus shared `Embedder` and `RetryableError` contracts with `IsRetryableVectorError` classification logic; added `vector_contracts_test.go` coverage and verified with `go test ./src/internal/domain/indexing -run Vector -count=1` and `go test ./... -count=1`.
- [x] Add `src/internal/orchestration/embeddings/ollama.go` implementing batched embedding requests with context timeout handling and `bge-m3` dimension checks.
  - Completed: Added `OllamaEmbedder` with configurable client/batch size, `/api/embed` request batching, timeout-aware per-call context budgeting, retry-classified transport/HTTP errors, malformed response guards, and strict `bge-m3` dimension validation (`1024`); validated with `go test ./src/internal/orchestration/... -count=1`.
- [x] Add `src/internal/orchestration/embeddings/ollama_test.go` for success path, timeout behavior, malformed payload handling, and dimension mismatch handling, then run `go test ./src/internal/orchestration/... -run Ollama -count=1`.
  - Completed: Added `ollama_test.go` with `TestOllamaEmbedderSuccessPath`, `TestOllamaEmbedderTimeoutBehavior`, `TestOllamaEmbedderMalformedPayloadHandling`, and `TestOllamaEmbedderDimensionMismatchHandling`, including retryability assertions; `go test ./src/internal/orchestration/... -run Ollama -count=1` passed.
- [x] Add `src/internal/storage/vector/sqlite/adapter.go` implementing sqlite vector bootstrap, `Upsert`, `Query`, `Delete`, `DeleteNamespace`, and `Health` with deterministic ordering guarantees.
  - Completed: Added a new SQLite vector backend adapter with schema bootstrap (`vector_records` table + namespace index + WAL pragmas), context-aware implementations for `Upsert`/`Query`/`Delete`/`DeleteNamespace`/`Health`, cosine similarity scoring over stored embeddings, and deterministic result ordering via explicit score/id/path/chunk/repo tie-break sorting; validated with `go test ./src/internal/storage/... -count=1` and `go test ./... -count=1`.
